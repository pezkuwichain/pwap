...
export default function NFTGalleryScreen() {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Transfer NFT Bottom Sheet */}
      <BottomSheet
        visible={transferVisible}
        onClose={() => { setTransferVisible(false); setTransferAddress(''); }}
        title={`Transfer ${selectedNFT?.name || 'NFT'}`}
      >
        <Input
        ...
        <Button
          ...
          style={{ marginTop: 16 }}
          onPress={async () => {
            if (!api || !selectedAccount || !selectedNFT || !transferAddress) return;
            setTransferring(true);
            try {
              const keyPair = await getKeyPair(selectedAccount.address);
              if (!keyPair) throw new Error('No keypair');
              // For tiki NFTs, use tiki pallet transfer if available
              // For generic NFTs, use uniques/nfts pallet
              // This is a placeholder - actual implementation depends on chain's NFT pallet
              logger.warn('[NFT] Transfer not yet implemented for this NFT type');
              Alert.alert('Info', 'NFT transfer will be available when the chain NFT pallet is ready.');
            } catch (e) {
