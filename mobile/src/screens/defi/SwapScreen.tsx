...
const SwapScreen: React.FC = () => {
  ...
  const fetchBalances = useCallback(async () => {
    ...
    try {
      ...
      logger.error('Failed to fetch from balance:', _error);
      setFromBalance('0');
    }
    ...
    try {
      ...
      logger.error('Failed to fetch to balance:', _error);
      setToBalance('0');
    }
  ...
  useEffect(() => {
    if (api && isApiReady) {
      ...
      if (!hasAssetConversion && __DEV__) {
        logger.warn('AssetConversion pallet not available in runtime');
      }
    }
  }, [api, isApiReady]);
        ...
        if (poolInfo && !poolInfo.isEmpty) {
          try {
            ...
            if (reserve0Data?.balance && reserve1Data?.balance) {
              ...
              logger.warn('Pool reserves found:', { reserve0, reserve1, asset1, asset2 });
              ...
              logger.warn('Pool has no reserves');
              setExchangeRate(0);
              setPoolReserves(null);
            }
          } catch (err) {
            logger.error('Error deriving pool account:', err);
            setExchangeRate(0);
            setPoolReserves(null);
          }
        } else {
          logger.warn('No liquidity pool found for this pair');
          setExchangeRate(0);
          setPoolReserves(null);
        }
      } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.scrollContent} contentContainerStyle={styles.scrollContentContainer} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchBalances().finally(() => setRefreshing(false)); }} />}>
        {/* Settings Button Row */}
        <View style={styles.settingsRow}>
          <TouchableOpacity onPress={() => setShowSettings(true)} style={styles.settingsButton} accessibilityRole="button" accessibilityLabel="Swap settings">
        ...
        <View style={styles.tokenCard}>
          ...
          <View style={styles.tokenInputRow}>
            <TextInput
              ...
              accessibilityLabel="From token amount"
            />
            <TouchableOpacity
            ...
            >
              <Image source={fromToken.logo} style={styles.tokenLogo} resizeMode="contain" accessibilityLabel={`${fromToken.symbol} logo`} />
              <Text style={styles.tokenSymbol}>{fromToken.symbol}</Text>
              <Text style={styles.tokenSelectorArrow}>▼</Text>
            </TouchableOpacity>
          </View>

          <TouchableOpacity style={styles.maxButton} onPress={handleMaxClick} accessibilityRole="button" accessibilityLabel="Use maximum balance">
        ...
        <View style={styles.swapDirectionContainer}>
          <TouchableOpacity style={styles.swapDirectionButton} onPress={handleSwapDirection} accessibilityRole="button" accessibilityLabel="Swap token direction">
        ...
        <View style={styles.tokenCard}>
          ...
          <View style={styles.tokenInputRow}>
            <TextInput
              ...
              value={toAmount}
              editable={false}
              accessibilityLabel="To token amount"
            />
            <TouchableOpacity
              style={styles.tokenSelector}
            ...
            >
              <Image source={toToken.logo} style={styles.tokenLogo} resizeMode="contain" accessibilityLabel={`${toToken.symbol} logo`} />
        ...
        <View style={styles.detailsCard}>
          ...
          <View style={styles.detailRow}>
            ...
            <Text style={styles.detailValue}>
              {fromToken.symbol === 'HEZ' || toToken.symbol === 'HEZ'
                ? `${fromToken.symbol} → ${fromToken.symbol === 'HEZ' ? 'wHEZ' : fromToken.symbol} → ${toToken.symbol === 'HEZ' ? 'wHEZ' : toToken.symbol} → ${toToken.symbol}`
          ...
          {/* Fee Breakdown */}
          <View style={styles.detailRow}>
            <Text style={styles.detailLabel}>LP Fee (3%)</Text>
            <Text style={styles.detailValue}>
              {fromAmount && parseFloat(fromAmount) > 0
                ? `${(parseFloat(fromAmount) * 0.03).toFixed(6)} ${fromToken.symbol}`
                : '-'}
            </Text>
          </View>
          <View style={styles.detailRow}>
          ...
          {/* Price Impact */}
          {fromAmount && poolReserves && parseFloat(fromAmount) > 0 && (
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Price Impact</Text>
              <Text style={[
                styles.detailValue,
                { color: parseFloat(fromAmount) / (poolReserves.reserve0 || 1) > 0.05 ? '#DC2626' : '#16A34A' }
              ]}>
                {(parseFloat(fromAmount) / (poolReserves.reserve0 || 1) * 100).toFixed(2)}%
              </Text>
            </View>
          )}
        </View>
        ...
        {fromAmount && poolReserves && parseFloat(fromAmount) / (poolReserves.reserve0 || 1) > 0.05 && (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            {TOKENS.map((token) => (
              ...
              >
                <Image source={token.logo} style={styles.tokenOptionLogo} resizeMode="contain" accessibilityLabel={`${token.symbol} logo`} />
            ...
            <TouchableOpacity style={styles.modalCloseButton} onPress={() => setShowTokenSelector(null)} accessibilityRole="button" accessibilityLabel="Cancel token selection">
      ...
      <Modal visible={showSettings} transparent animationType="slide" onRequestClose={() => setShowSettings(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.slippageButtons}>
              {[0.1, 0.5, 1.0, 2.0].map((val) => (
                <TouchableOpacity
                  ...
                  accessibilityRole="button"
                  accessibilityLabel={`Set slippage to ${val} percent`}
                >
            ...
            <TouchableOpacity style={styles.modalCloseButton} onPress={() => setShowSettings(false)} accessibilityRole="button" accessibilityLabel="Close settings">
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.confirmButtons}>
              <TouchableOpacity style={styles.confirmCancelButton} onPress={() => setShowConfirm(false)} accessibilityRole="button" accessibilityLabel="Cancel swap">
                <Text style={styles.confirmCancelButtonText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.confirmSwapButton} onPress={handleConfirmSwap} accessibilityRole="button" accessibilityLabel="Confirm swap">
