...
const SettingsScreen: React.FC = () => {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      logger.warn('Error fetching profile:', _err);
    } finally {
  ...
  // 5. Network Switcher
  const handleNetworkChange = async (network: 'pezkuwi' | 'dicle' | 'zagros' | 'zombienet') => {
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              logger.error('Error retrieving mnemonic:', error);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]} testID="settings-screen">
      ...
      <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchProfile().finally(() => setRefreshing(false)); }} />}>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            ...
            <TouchableOpacity
              ...
              accessibilityRole="button"
              accessibilityLabel={`Select ${NETWORKS.pezkuwi.displayName} network`}
            >
            ...
            <TouchableOpacity
              style={[styles.networkOption, currentNetwork === 'dicle' && styles.selectedNetwork]}
              onPress={() => handleNetworkChange('dicle')}
              testID="network-option-testnet"
              accessibilityRole="button"
              accessibilityLabel={`Select ${NETWORKS.dicle.displayName} network`}
            >
              <Text style={styles.networkName}>{NETWORKS.dicle.displayName}</Text>
              <Text style={styles.networkUrl}>{NETWORKS.dicle.rpcEndpoint}</Text>
            </TouchableOpacity>

            <TouchableOpacity
              style={[styles.networkOption, currentNetwork === 'zagros' && styles.selectedNetwork]}
              onPress={() => handleNetworkChange('zagros')}
              testID="network-option-canary"
              accessibilityRole="button"
              accessibilityLabel={`Select ${NETWORKS.zagros.displayName} network`}
            >
              <Text style={styles.networkName}>{NETWORKS.zagros.displayName}</Text>
              <Text style={styles.networkUrl}>{NETWORKS.zagros.rpcEndpoint}</Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              accessibilityRole="button"
              accessibilityLabel={`Select ${NETWORKS.zombienet.displayName} network`}
            >
